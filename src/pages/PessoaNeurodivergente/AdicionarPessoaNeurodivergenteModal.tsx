...
interface AdicionarPessoaNeurodivergenteModalProps {
  open: boolean;
  onClose: () => void;
}

const AdicionarPessoaNeurodivergenteModal: React.FC<AdicionarPessoaNeurodivergenteModalProps> = ({ open, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    const novoPessoaNeurodivergente = {
      nome,
      descricao,
    };

    try {
      await axios.post(`${urlServidor}/neurodivergente`, novoPessoaNeurodivergente, { headers });
      toast.success('Pessoa Neurodivergente adicionada com sucesso!');
      setTimeout(() => {
        onClose();
      }, 1000);
    } catch (error: unknown) {
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        <Typography variant="h6" component="h2">
          Adicionar Pessoa Neurodivergente
        </Typography>
        <IconButton
          onClick={onClose}
          style={{
...
export default AdicionarPessoaNeurodivergenteModal;
