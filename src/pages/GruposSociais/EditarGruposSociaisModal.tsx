...
interface EditarGruposSociaisModalProps {
  open: boolean;
  onClose: () => void;
  gruposSociais: GruposSociaisInterface | null;
}

const EditarGruposSociaisModal: React.FC<EditarGruposSociaisModalProps> = ({ open, onClose, gruposSociais }) => {
    ...
    useEffect(() => {
      if (gruposSociais) {
        setNome(gruposSociais.nome);
        setDescricao(gruposSociais.descricao);
        setErroNome(false);
    ...
    const handleSave = async () => {
        ...
        try {
          await axios.put(`${urlServidor}/grupossociais/${gruposSociais?.id}`, gruposSociaisAtualizado, { headers });
          toast.success('Grupo Social atualizado com sucesso!');
          setTimeout(() => {
            onClose();
          }, 1000);
        } catch (error: unknown) {
  ...
  export default EditarGruposSociaisModal;
