...
interface AdicionarGruposSociaisModalProps {
  open: boolean;
  onClose: () => void;
}

const AdicionarGruposSociaisModal: React.FC<AdicionarGruposSociaisModalProps> = ({ open, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    const novoGruposSociais = {
      nome,
      descricao,
    };

    try {
      await axios.post(`${urlServidor}/grupossociais`, novoGruposSociais, { headers });
      toast.success('Grupo Social adicionado com sucesso!');
      setTimeout(() => {
        onClose();
      }, 1000);
    } catch (error: unknown) {
...
export default AdicionarGruposSociaisModal;
