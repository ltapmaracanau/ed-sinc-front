...
interface EditarPessoaComDeficienciaModalProps {
  open: boolean;
  onClose: () => void;
  pessoaComDeficiencia: PessoaComDeficienciaInterface | null;
}

const EditarPessoaComDeficienciaModal: React.FC<EditarPessoaComDeficienciaModalProps> = ({ open, onClose, pessoaComDeficiencia }) => {
    ...
    useEffect(() => {
      if (pessoaComDeficiencia) {
        setNome(pessoaComDeficiencia.nome);
        setDescricao(pessoaComDeficiencia.descricao);
        setErroNome(false);
    ...
    const handleSave = async () => {
        ...
        const pessoaComDeficienciaAtualizado = {
          id: pessoaComDeficiencia?.id,
          nome,
          descricao,
        };
    
        try {
          await axios.put(`${urlServidor}/pcd/${pessoaComDeficiencia?.id}`, pessoaComDeficienciaAtualizado, { headers });
          toast.success('Pessoa Com Deficiência atualizada com sucesso!');
          setTimeout(() => {
            onClose();
          }, 1000);
        } catch (error: unknown) {
  ...
  export default EditarPessoaComDeficienciaModal;
