...
interface AdicionarPessoaComDeficienciaModalProps {
  open: boolean;
  onClose: () => void;
}

const AdicionarPessoaComDeficienciaModal: React.FC<AdicionarPessoaComDeficienciaModalProps> = ({ open, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    const novoPessoaComDeficiencia = {
      nome,
      descricao,
    };

    try {
      await axios.post(`${urlServidor}/pcd`, novoPessoaComDeficiencia, { headers });
      toast.success('Pessoa Com Deficiência adicionada com sucesso!');
      setTimeout(() => {
        onClose();
      }, 1000);
    } catch (error: unknown) {
    ...
    <Modal open={open} onClose={onClose}>
      ...
      >
        <Typography variant="h6" component="h2">
          Adicionar Pessoa Com Deficiência
        </Typography>
        <IconButton
          onClick={onClose}
          style={{
...
export default AdicionarPessoaComDeficienciaModal;
